...
Use OAuth when you want browser-based sign-in and don’t want to manually handle tokens. Skip to task 4.

---

### Option B: Generate a Bearer Token Using cURL
...
5. Use the following example as a template for MCP server configuration and replace the code in `cline_mcp_setting.json`. Choose from:
    ...
    ### Option A: For Oauth authentication
